...
- [ ] The semantic `--color-success` and `--color-error` tokens from the design reference are added. No existing brand color or typography token is modified, and no new brand color is introduced.
- [ ] An empty submission displays `Email address is required`.
- [ ] A malformed address displays `Please enter a valid email address`.
- [ ] Validation applies the established basic email-shape rule to the trimmed value, so leading and trailing whitespace is ignored.
- [ ] Validation feedback appears before any request would be issued.
- [ ] Feedback is exposed through an accessible live status region and is announced when it changes.
- [ ] Success and error are distinguishable without color, through text and a non-color cue, in addition to the semantic colors.
- [ ] Feedback blocks are square and bordered, consistent with the established card treatment. They are not rounded and not pills.
- [ ] The idle, submitting, success, and error treatments are all reachable by interacting with the form, so a reviewer can see each one. Empty and malformed submissions exercise validation variants of the error treatment rather than separate submission states.
- [ ] The submitting treatment disables the action and changes its label to `Subscribing…`.
- [ ] Changing the field value clears both success and error feedback on the first change, without requiring another submission.
- [ ] A valid prototype submission issues no network request to the ATF email service.
- [ ] The prototype success treatment displays the real production success copy rather than hedged preview copy.
- [ ] The non-live review marker remains present and remains reachable independently of the feedback copy.
- [ ] The prototype-only submission mechanic is isolated to one place and is trivially removable, with no hidden production fallback.
- [ ] The simulated pending operation lasts 750 milliseconds, making the submitting treatment observable during manual review; automated tests use fake timers, and no real-time wait is added to the suite.
- [ ] The approved section composition, colors, geometry, and copy from the previous ticket are unchanged.
- [ ] Rendered-application tests cover the empty and malformed feedback paths, establish that native browser constraint validation does not preempt the application messages, verify the absence of any service call on a valid prototype submission, verify the presence of the review marker, and verify the clearing of stale feedback on the first field change.
